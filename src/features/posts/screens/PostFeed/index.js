...
import {handlePostReaction, getLikeDetails} from '../../../../lib/postreactions/postreactionhandlers';
    ...
    <>
      {routeMapDetail && (
        <FlatList
          ...
          renderItem={({item}) => {
            let likeData = getLikeDetails(item,seekerId);
            console.log("item in seeker post: ", item);
            return (
            <SeekerPost
              post={item}
              likeData={likeData}
